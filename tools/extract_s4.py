...
# A trained person-segmentation model cuts a clean silhouette regardless of
# how dark his suit is or how bright the card-glow behind him gets — far more
# reliable here than colour/darkness-based GrabCut heuristics.
from PIL import Image as _PILImage
from rembg import new_session as _new_session, remove as _remove

x0, y0, x1, y1 = PERSON
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
_session = _new_session("u2net_human_seg")
_seg = _remove(_PILImage.fromarray(_rgb), session=_session, only_mask=True)
alpha = np.array(_seg)
_, alpha = cv2.threshold(alpha, 20, 255, cv2.THRESH_BINARY)
alpha = cv2.morphologyEx(alpha, cv2.MORPH_CLOSE,
                         cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)))
# keep only the box around him, with a little air, same as before
mask_box = np.zeros_like(alpha)
mask_box[y0:y1, x0:x1] = 255
alpha = cv2.bitwise_and(alpha, mask_box)
